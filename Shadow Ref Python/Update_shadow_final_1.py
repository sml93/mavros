...
class Straight_Inspection:
    ...
    def __init__(self):
        ...
        while not rospy.is_shutdown(): #subsequent functions below are automatically run using this while loop (inert functions)  
            ...
            #print "45: ", self.sensor_45, "alti: ", abs(self.local_position.pose.position.z), "Counter: ", self.counter, "Counter2: ", self.counter_2, "Counter3: #", self.counter_3, "Counter4: ", self.counter_4, "Z: ", self.z
            
            #print "X: ", self.x, "Y: ", self.y
            #while not self.x or not self.y:
            #    time.sleep(0.01)

            if self.z_increment < 0.6:
    ...
    def top_right_bot(self):
        ...
        if self.counter_2 < 4:    
            ...
            if self.sensor_45 <= 2.00: # third wall
    ...
    def estimator(self):
        ...
        if self.left_est == 1.0 && self.top_est == 0.0 && self.right_est == 0.0 && self.bot_est == 0.0: 
        ...
        elif self.left_est == 0.0 && self.top_est == 1.0 && self.right_est == 0.0 && self.bot_est == 0.0: 
        ...
        elif self.left_est == 0.0 && self.top_est == 0.0 && self.right_est == 1.0 && self.bot_est == 0.0: 
            
            if self.sensor_180 == self.x:
                pos.pose.position.x = 0.0
            else:
                pos.pose.position.x = -(self.x - self.sensor_180) * self.kp_est

            if self.sensor_270 == self.y:
                pos.pose.position.y = 0.0 
            else:
                pos.pose.position.y = -(self.y - self.sensor_270) * self.kp_est
                
        elif self.left_est == 0.0 && self.top_est == 0.0 && self.right_est == 0.0 && self.bot_est == 1.0: 
            
            if self.sensor_270 == self.x:
                pos.pose.position.x = 0.0
            else:
                pos.pose.position.x = -(self.x - self.sensor_270) * self.kp_est

            if self.sensor_0 == self.y:
                pos.pose.position.y = 0.0 
            else:
                pos.pose.position.y = -(self.y - self.sensor_0) * self.kp_est
